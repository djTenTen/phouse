	...
	<?= form_open("expense/update/".$eID)?>
		...
		<div class="row">
			<div class="form-group col-lg-4">
				...
				<input type="text" class="form-control" name="payee" value="<?= $expdetails['payee']?>" required autofocus>
			</div>

			<div class="form-group col-lg-4">
				<label class="d-block">Payment Receiver</label>
				<input type="text" class="form-control" name="receiver" value="<?= $expdetails['receiver']?>">
			</div>
						...
						<div class="row">
							<div class="col-4">
								<select name="mme" class="form-control" required>
                                    <option value="<?= $edate[1];?>" selected><?= date("F",mktime(0, 0, 0, $edate[1], 10));?></option>
							...
							<div class="col-3">
								<select name="dde" class="form-control" required>
                                    <option value="<?= $edate[2];?>" selected><?= $edate[2];?></option>
							...
							<div class="col-4">
								<select name="yye" class="form-control" required>
                                    <option value="<?= $edate[0];?>" selected><?= $edate[0];?></option>
				...
				<table class="table table-bordered" data-max-row="14" id="expense">
						...
						<tr>
							<td class="align-middle">
								<select class="custom-select" name="expense-branch[]" required>
									<option value="<?= $encrypter->encrypt($ei['branch_id']);?>" selected><?= $ei['branch'];?></option>
							...
							<td class="align-middle">
								<select class="custom-select" name="expense-category[]" required>
									<option value="<?= $encrypter->encrypt($ei['expense_category_id']);?>" selected><?= $ei['category'];?></option>
									<?php foreach($expensecategory as $ec){?>				
									<option value="<?= $encrypter->encrypt($ec['expense_category_id']);?>"><?= $ec['name'];?></option>
									<?php }?>
								</select>
							</td>
							<td class="align-middle"><input type="text" class="form-control" name="expense-description[]" value="<?= $ei['description'];?>" required></td>
							<td class="align-middle"><input type="number" class="form-control" data-name="amount" name="expense-amount[]" step="0.01" value="<?= $ei['amount'];?>" required></td>
					...
					<tfoot>
						<tr class="tx-20">
							...
							<td class="align-middle text-right" data-name="total"><?= $totalei;?></td>
							<td></td>
						</tr>
					</tfoot>
				</table>
				...
				<table class="table table-bordered" data-max-row="10" id="payment">
					...
					<tbody id="tpayment">
                        <?php
                            ...
                            $totalep += $ep['amount'];
                            $pdate = explode('-', $ep['date']);
                        ?>
						<tr>
							<td class="align-middle">
								<select class="custom-select" name="payment-source[]">
									<option value="<?= $encrypter->encrypt($ep['bank_account_id']);?>" selected>[<?= $ep['bank'];?>]<?= $ep['bank_account'];?></option>
							...
							<td class="align-middle">
								<select class="custom-select" name="payment-type[]">
                                    <option value="<?= $ep['type'];?>" selected><?= $ep['type'];?></option>
								...
								<div class="row">
									<div class="col-4">
										<select name="mmp[]" class="form-control" required>
                                            <option value="<?= $pdate[1];?>" selected><?= date("F",mktime(0, 0, 0, $pdate[1], 10));?></option>
											<option value="01">January</option>
											<option value="02">February</option>
											<option value="03">March</option>
											<option value="04">April</option>
									...
									<div class="col-3">
										<select name="ddp[]" class="form-control" required>
                                            <option value="<?= $pdate[2];?>" selected><?= $pdate[2];?></option>
											<option value="01">01</option>
											<option value="02">02</option>
											<option value="03">03</option>
											<option value="04">04</option>
							...
							<td class="align-middle"><input type="text" class="form-control" name="payment-check-no[]" value="<?= $ep['check_number'];?>"></td>
							<td class="align-middle"><input type="number" class="form-control" data-name="amount" name="payment-amount[]" step="0.01" value="<?= $ep['amount'];?>"></td>
							<td><button class="btn btn-icon btn-danger btn-xs remove" type="button" data-action="remove"><i class="fas fa-times"></i></button></td>
						</tr>
                        <?php }?>
					</tbody>
					<tfoot>
						<tr class="tx-20">
							...
							<td class="align-middle text-right" data-name="total"><?= $totalep;?></td>
							<td></td>
						</tr>
					</tfoot>
				</table>
		...
		<div class="mg-t-20">
			<button class="btn btn-primary mg-r-15" name="publish" type="submit">Update Expense</button>
		</div>
